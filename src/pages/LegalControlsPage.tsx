...
import {
  createLegalControl,
  formatDate,
  getLegalEvidenceSignedUrl,
  isLegalControlOverdue,
  listLegalControls,
  listLegalEntities,
  markLegalControlReviewed,
  syncIncompleteLegalTasksToTodo,
  updateLegalControl,
  uploadLegalControlEvidence,
} from '../lib/legalApi';
import { isAuditControlDueSoon } from '../lib/auditControlUtils';
import {
  LEGAL_AREAS,
  LEGAL_COMPLIANCE_CADENCES,
  LEGAL_COMPLIANCE_CADENCE_LABELS,
  LEGAL_CONTROL_SOURCE_LABELS,
  LEGAL_CONTROL_SOURCES,
  LEGAL_CONTROL_STATUS_LABELS,
  LEGAL_CONTROL_STATUSES,
  type LegalComplianceCadence,
  type LegalControl,
  type LegalControlSource,
  type LegalControlStatus,
} from '../lib/legalTypes';
...
/**
 * Corporate legal audit matrix (parent + each portfolio entity).
 * Lives only under /sales/legal — never on entity-detail pages.
 */
export function LegalControlsPage({ salesUser }: Props) {
  const [controls, setControls] = useState<LegalControl[]>([]);
  ...
  const [source, setSource] = useState<LegalControlSource | 'all'>('all');
  const [status, setStatus] = useState<LegalControlStatus | 'all'>('all');
  const [q, setQ] = useState('');

  const [title, setTitle] = useState('');
  const [entityId, setEntityId] = useState('');
  const [cadence, setCadence] = useState<LegalComplianceCadence>('annual');
  const [due, setDue] = useState('');
  const [notes, setNotes] = useState('');
  const [newArea, setNewArea] = useState('Organizational');
        ...
        if (mounted) {
          setError(
            err instanceof Error
              ? err.message
              : 'Failed to load legal controls (run migration 0026)',
          );
        }
      } finally {
  ...
  const overdueCount = useMemo(
    () => filtered.filter((c) => isLegalControlOverdue(c)).length,
    [filtered],
  );
  ...
  async function patchControl(
    id: string,
    patch: Parameters<typeof updateLegalControl>[1],
  ) {
    setError(null);
    if (patch.status) {
      setControls((prev) =>
    ...
    try {
      await updateLegalControl(id, patch);
      await refresh();
    } catch (err) {
  ...
  async function onMarkReviewed(id: string) {
    ...
    try {
      await markLegalControlReviewed(id, salesUser.id);
      await refresh();
    } catch (err) {
  ...
  async function onSyncTasks() {
    ...
    try {
      ...
      setNotice(
        `Tasks ready: ${result.legalCreated} new legal task(s), ${result.todoCreated} pushed to Tage · Legal To Do.`,
      );
    } catch (err) {
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h1>Corporate audit</h1>
          <p className="muted">
            Checklist controls for Tage parent and each subsidiary. Incomplete items can
            become Legal tasks / Microsoft To Do items.
          </p>
        </div>
        ...
        <>
          <div className="toolbar hr-toolbar hr-compliance-filters">
            ...
            <select
              className="input"
              value={source}
              onChange={(e) => setSource(e.target.value as LegalControlSource | 'all')}
            >
              <option value="all">All sources</option>
              {LEGAL_CONTROL_SOURCES.map((s) => (
                <option key={s} value={s}>
                  {LEGAL_CONTROL_SOURCE_LABELS[s]}
                </option>
              ))}
            </select>
            <select
              className="input"
              value={status}
              onChange={(e) => setStatus(e.target.value as LegalControlStatus | 'all')}
            >
              <option value="all">All statuses</option>
              {LEGAL_CONTROL_STATUSES.map((s) => (
                <option key={s} value={s}>
                  {LEGAL_CONTROL_STATUS_LABELS[s]}
                </option>
              ))}
            </select>
          </div>
          ...
          <section className="panel ops-compliance-hub">
            ...
            {filtered.length === 0 ? (
              <p className="muted">
                ...
                <code>0026_legal_corporate_audit.sql</code> to seed the corporate audit.
              </p>
            ) : (
              <ul className="ops-compliance-list hr-control-list">
                {filtered.map((item) => {
                  const overdue = isLegalControlOverdue(item);
                    ...
                    >
                      <div className="hr-control-main">
                        ...
                        <div className="muted small">
                          ...
                          >
                            {LEGAL_CONTROL_SOURCE_LABELS[
                              (item.source as LegalControlSource) ?? 'manual'
                            ] ?? item.source}
                          </span>
                          {' · '}
                          {LEGAL_COMPLIANCE_CADENCE_LABELS[item.cadence]}
                          {' · '}
                          {LEGAL_CONTROL_STATUS_LABELS[item.status]}
                          {item.next_due_at
            ...
            <form className="form-grid" onSubmit={(e) => void onAdd(e)}>
              ...
              <label>
                ...
                <select
                  ...
                  value={cadence}
                  onChange={(e) =>
                    setCadence(e.target.value as LegalComplianceCadence)
                  }
                >
                  {LEGAL_COMPLIANCE_CADENCES.map((c) => (
                    <option key={c} value={c}>
                      {LEGAL_COMPLIANCE_CADENCE_LABELS[c]}
                    </option>
                  ))}
                </select>
              </label>
...
function ControlEditor({
  ...
  const [cadence, setCadence] = useState<LegalComplianceCadence>(item.cadence);
  ...
  const [status, setStatus] = useState<LegalControlStatus>(item.status);
  ...
  async function onFile(file: File | null) {
    ...
    onError(null);
    try {
      const result = await uploadLegalControlEvidence({ control: item, file });
      if (!result.ok) {
  ...
  async function openAttached() {
    ...
    const url = await getLegalEvidenceSignedUrl(item.evidence_storage_path);
    ...
    <div className="hr-control-editor form-grid">
      ...
      <label>
        ...
        <select
          className="input"
          value={status}
          onChange={(e) => setStatus(e.target.value as LegalControlStatus)}
        >
          {LEGAL_CONTROL_STATUSES.map((s) => (
            <option key={s} value={s}>
              {LEGAL_CONTROL_STATUS_LABELS[s]}
      ...
      <label>
        ...
        <select
          className="input"
          value={cadence}
          onChange={(e) => setCadence(e.target.value as LegalComplianceCadence)}
        >
          {LEGAL_COMPLIANCE_CADENCES.map((c) => (
            <option key={c} value={c}>
              {LEGAL_COMPLIANCE_CADENCE_LABELS[c]}
      ...
      <div className="muted small full">
        {item.last_reviewed_at
          ...
          : 'Not yet reviewed'}
        {' · '}
        Due dates are per company row (parent vs each entity). Mark reviewed rolls next due
        by frequency.
      </div>
      <div className="form-actions">
        <button
