...
| Graph mail scopes (2026-08-02) | **Delegated** `Mail.Send` + `Mail.ReadWrite` — admin consent granted for Tage Venture Capital |
| Existing secrets | `Tage OS Graph`, `Tage VC OS Supabase`, `supabase` (hints only; never commit secret values) |

### Permission model (what we chose)

**Delegated** for bootstrap (recommended now):

| Permission | Type | Why |
|------------|------|-----|
| `Mail.Send` | Delegated | Send as signed-in user / From=alias |
| `Mail.ReadWrite` | Delegated | Ingest AP/AR/W-9 replies from host mailbox |
| `Mail.Read` | Delegated | Least-privilege read if Write not desired later |
| Existing | `openid` `profile` `email` `offline_access` `User.Read` | Sign-in |

**Not using Application Mail.* yet** — those send/read *any* mailbox and need heavier admin consent + Exchange application access policies. Revisit when moving to true shared mailboxes.
...
1. **API permissions** — ensure Mail.Send + Mail.ReadWrite (Delegated) listed.
2. **Grant admin consent for Tage Venture Capital**.
