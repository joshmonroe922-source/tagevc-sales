...
| `VERIFIED_FIRST_API_KEY` | Optional legacy: `username:password`, or password if USERNAME set |
| `VERIFIED_FIRST_WEBHOOK_SECRET` | HMAC / shared secret for webhook |
| `VERIFIED_FIRST_API_BASE` | Optional API host (default `https://api1.verifiedfirst.com`; staging `https://api2.verifiedfirst.com`) |

**Auth model:** HTTP **Basic Authorization** (username + password) — **not Bearer, not OAuth** for the REST order API. Credentials are provisioned by VF Integrations after Staging/Test review (`integrations@verifiedfirst.com`). Public docs: [API docs](https://docs.api.verifiedfirst.com/#) · [Standard request formats](https://help.verifiedfirst.com/hc/en-us/articles/360021050472-Standard-API-Request-and-Response-Formats).

**Order call:** `POST {API_BASE}/external/verified-first/order` with `Authorization: Basic …` and `Content-Type: application/json`.
...
Asking VF for a single “API key” is the wrong ask — they don’t issue one for this API. Email Integrations for **Basic Auth username/password**, Staging activation, and whitelist (see copy-paste below).
...
- Vendor client aligned to Basic Auth + `api1` / `/external/verified-first/order` (`LIVE` still `0`)
...
3. **Package catalog IDs** (`package_id` UUIDs) from VF portal / `GET …/account/packages` → update `os_screening_packages.vendor_package_id`.
4. Approve flipping **`VERIFIED_FIRST_LIVE=1`** only after a Staging smoke order (keep `0` until then).

### Wire steps (after Staging credentials arrive)
...
printf '%s' "$VERIFIED_FIRST_API_USERNAME" | vercel env add VERIFIED_FIRST_API_USERNAME preview
printf '%s' "$VERIFIED_FIRST_API_PASSWORD" | vercel env add VERIFIED_FIRST_API_PASSWORD production
printf '%s' "$VERIFIED_FIRST_API_PASSWORD" | vercel env add VERIFIED_FIRST_API_PASSWORD preview
# Staging host until VF clears production
printf '%s' 'https://api2.verifiedfirst.com' | vercel env add VERIFIED_FIRST_API_BASE production
printf '%s' 'https://api2.verifiedfirst.com' | vercel env add VERIFIED_FIRST_API_BASE preview
...
- Confirm remaining order-body FCRA / subject fields with VF implementation packet before LIVE=1
