...
- **Exclusive expand (default UX):** opening a group collapses other open groups at the same level (top-level peers, and nested peers under the same parent). Shared `AppSidebar` — parent Tage OS + R619 / Signent / Instant NDA / future entity shells inherit
- Active child route forces parent open on navigation (nested: Performance Management / Screening keeps Shared Services + HR expanded; Grow children keep Grow expanded; IT Activity / Audit keeps Shared Services + IT expanded; Document Library / DocuSign keeps Shared Services + Admin expanded). Deep-link / active-route auto-open is additive and may leave another manually opened section open until the next user toggle
