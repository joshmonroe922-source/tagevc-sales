...
export function canSendMarketing(input: {
  ...
  /** Alias used by dialer/enrollment callers */
  ...
  if (input.killSwitch) {
    return {
      allow: false,
      reason: 'Entity kill switch active',
      code: 'KILL_SWITCH',
    };
  }
  ...
  if (input.suppressed) {
    return {
      allow: false,
      reason: 'Email on suppression list',
      code: 'SUPPRESSED',
    };
  }
  if (input.conversing) {
    return {
      allow: false,
      reason: 'Contact in active conversation',
      code: 'CONVERSING',
    };
  }
  const perm = (input.emailPermission ||
    input.permission ||
    'opted_in') as EmailPermission;
  if (perm === 'opted_out') {
    return {
      allow: false,
