...
export type MutexPolicy = {
  /** Max active enrollments per mutex_group (default 1). */
  maxPerGroup: number;
  /** Max total active sequences (default 3). */
  maxGlobal: number;
...
export function checkMutex(input: {
  ...
  if (input.nextMutexGroup) {
    const same = input.active.filter((a) => a.mutexGroup === input.nextMutexGroup);
