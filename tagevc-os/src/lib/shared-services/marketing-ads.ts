...
async function syncLinkedInAdsStub(
  ...
  // Structure-only live path — full Analytics Finder is Phase 31+
  const sb = await createPersistClient();
  await sb.from('os_marketing_analytics_events').insert({
    event_id: `PAD-${Date.now().toString(36)}`,
    kind: 'engagement',
    ...
    metrics: {
      ...
      impressions: 0,
      clicks: 0,
      note: 'LinkedIn Ads token present — insights query deferred',
    },
    occurred_at: new Date().toISOString(),
  });
  return {
    campaign_id: campaignId,
    ok: true,
    detail: 'LinkedIn Ads token ok · insights deferred to Phase 31',
    impressions: 0,
    clicks: 0,
  };
}
