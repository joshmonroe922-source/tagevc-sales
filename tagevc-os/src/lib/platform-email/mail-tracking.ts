/** Custom open/click tracking for Graph-sent platform mail (Node runtime). */
...
export function mailTrackingBaseUrl(appUrl?: string): string {
  const base = (
    appUrl ??
    ...
    .trim()
    .replace(/\/$/, '');
  return `${base}/api/platform-email/mail-tracking`;
}
