...
const requestSchema = z.discriminatedUnion('action', [
  ...
  z.object({
    action: z.literal('announce_ed25519_rotation'),
    previous_key_id: z
      .string()
      .trim()
      .min(3)
  ...
  z.object({
    action: z.literal('activate_dual_key'),
    rotation_id: z.uuid(),
  }),
  z.object({
  ...
  z.object({
    action: z.literal('record_oncall_ack'),
    delivery_id: z.uuid(),
  ...
  z.object({
    action: z.literal('escalate_phase51_page_failures'),
  }),
  z.object({
    action: z.literal('record_phase51_required_check_verification'),
    branch_name: z
      .string()
      .trim()
      .toLowerCase()
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      phase51PageFailureEscalations: phase51.phase51PageFailureEscalations,
      phase51RequiredCheckVerifications:
        phase51.phase51RequiredCheckVerifications,
  ...
  try {
    ...
    switch (parsed.data.action) {
      ...
      case 'schedule_phase44_canary':
        ...
        break;
      case 'announce_ed25519_rotation':
        result = await announceSnapshotEd25519RotationPhase45({
          actorId: gate.profile.id,
          previousKeyId: parsed.data.previous_key_id,
          nextKeyId: parsed.data.next_key_id,
        });
        break;
      case 'activate_dual_key':
        result = await activateSnapshotDualKeyPhase45({
          actorId: gate.profile.id,
      ...
      case 'record_cutover_acceptance':
        ...
        break;
      case 'record_oncall_ack':
        result = await recordSnapshotOncallAckPhase47({
          actorId: gate.profile.id,
      ...
      case 'record_soak_status':
        ...
        break;
      case 'escalate_phase51_page_failures':
        result = await escalateSnapshotPhase51PageDeliveryFailures({
          actorId: gate.profile.id,
        });
        break;
      case 'record_phase51_required_check_verification':
        result = await recordSnapshotPhase51RequiredCheckVerification({
          actorId: gate.profile.id,
