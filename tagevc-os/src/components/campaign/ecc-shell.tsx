...
import { isPathMatch } from '@/lib/platform/shell/nav-active';
  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl border border-[#d7d3c3] bg-gradient-to-br from-[#ece9e6] via-[#e8e4de] to-[#d7d3c3] px-5 py-6 sm:px-7">
        <div
          className="pointer-events-none absolute -right-16 -top-20 h-56 w-56 rounded-full bg-[#9f957c]/20 blur-3xl"
          aria-hidden
        />
        ...
        <p className="mt-2 max-w-xl text-sm text-[#7c7871]">
          Owned campaign platform for Tage + subsidiaries. Graph for 1:1 ·
          controlled bulk with Reply-To + tracking.
        </p>
        {entityLabel ? (
          <p className="mt-3 text-xs font-medium text-[#535c63]">{entityLabel}</p>
        ) : null}
      </div>
      <nav
        className="flex flex-wrap gap-1 border-b border-[#d7d3c3] pb-px"
        aria-label="Email Campaign Center"
      >
        {NAV.map((item) => {
          const exact = 'exact' in item && item.exact;
          const active = isPathMatch(pathname, item.href, { exact });
          return (
            <Link
              key={item.href}
              href={item.href}
              ...
              className={cn(
                ...
                active
                  ? 'border border-b-white border-[#d7d3c3] bg-white font-medium text-[#3a414f]'
                  : 'text-[#7c7871] hover:text-[#3a414f]',
              )}
            >
              {item.label}
            </Link>
