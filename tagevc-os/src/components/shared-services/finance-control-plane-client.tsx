    ...
    <div className="space-y-8">
      <header className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline">Phase 55</Badge>
          <Badge variant="secondary">Feed · {report.feed_status}</Badge>
          <Badge variant="outline">IES system of record</Badge>
          <Badge variant="outline">Money auto-approve · never</Badge>
        </div>
      ...
      <section className="space-y-3">
        ...
        {report.checklist.length === 0 ? (
          <EmptyState
            title="No close items yet"
            description="Refresh the board to seed month-end checklist stubs, or wait for IES feed wiring."
          />
        ) : (
          <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {report.checklist.map((item) => (
                    <TableRow key={`${item.item_key}-${item.status}`}>
                      <TableCell>
                  ...
                  {report.writeback_proposals.map((p) => (
                    <TableRow key={p.proposal_id}>
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {p.entity_id ?? 'Firm'} · {p.proposal_id.slice(0, 8)}
                        </div>
                      </TableCell>
